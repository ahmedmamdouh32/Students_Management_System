...
search_input = ''  #--important variable--
...
# Bind event to handle the key press
search_entry.bind("<Key>", search_entry_on_key_press)
search_entry.bind("<BackSpace>", search_entry_on_backspace)
######################################################################################
...
button_about_me = Button(text = "About",font="arial 30",fg="#06275c",bg="#eed055")

######################  Load DataBase Data    ######################
update_table()
####################################################################
for i in range(210,310):
    root.after(10,lambda i:i+1)
    button_reload.place(x=1180,y=i)
root.mainloop()
